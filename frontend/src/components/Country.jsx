...
function Country() {
  ...
  async function getCovidData() {
    const response = await fetch(
      `${process.env.REACT_APP_HOST}/api/covid/countryspecific`,
      {
        method: "POST",
        headers: {
          Accept: "*/*",
  ...
  return (
    <>
      <div className="bg-gradient-to-r from-sky-400 via-sky-200 to-cyan-100 text-center">
        <a className="relative block p-8 overflow-hidden border border-gray-100 rounded-lg">
          <span className="absolute inset-x-0 bottom-0 h-1  bg-gradient-to-r from-blue-700 via-sky-500 to-blue-400"></span>
          ...
          <div className="justify-center gap-5 sm:flex">
            <div>
              ...
              <p className="mt-1 text-xs font-medium text-gray-600">
                {data.countryInfo.iso2} | {data.continent}
              </p>
            </div>
          ...
          <dl className="flex flex-wrap md:order-2 gap-x-2 gap-y-6 justify-center mt-6">
            ...
            <div className="flex flex-col-reverse ml-3 sm:ml-6">
              <dt className="text-2xl font-bold text-gray-600">
                {data.recovered}
              </dt>
      ...
      <div className="flex justify-center items-center gap-2 border-sky-200 rounded bg-sky-800">
        ...
        <button
          type="button"
          className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none  font-medium rounded text-sm px-5 py-1 mt-6 text-center mr-3 md:mr-0 focus:ring-blue-800 shadow-xl"
          onClick={handleClick}
        >
          Submit
        </button>
      ...
      <div>
        <LastDays
          name={data.country}
          iniDate={selectedDate}
          finDate={selectedDate1}
          clickk={clickk}
        />
      </div>
    </>
